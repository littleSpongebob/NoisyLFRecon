...
def bilinear_sampler_1d_h(input_images, x_offset, wrap_mode='border', name='bilinear_sampler', **kwargs):
    ...
    def _interpolate(im, x, y):
        with tf.variable_scope('_interpolate'):
            ...
            x = tf.clip_by_value(x, 0.0,  _width_f - 1 + 2 * _edge_size) #set x value between 0 and _(width_f - 1 + 2 * _edge_size)
    ...
    with tf.variable_scope(name):
        _num_batch    = input_images.shape.as_list()[0]
        _height       = input_images.shape.as_list()[1]
        _width        = input_images.shape.as_list()[2]
        _num_channels = input_images.shape.as_list()[3]

        _height_f = tf.cast(_height, tf.float32) #"tf.cast" change data type
